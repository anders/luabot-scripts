-- JSON4Lua modified for d^Hluabot
--
-- changes:
-- no module(), made encode/decode/null local, return table with encode/decode/null
-- 2012-05-21: add some aliases: json.dump, json.load, etc
...
function decode_scanString(s,startPos)
  ...
  local stringEval = stringValue:gsub("\\(.)(%w?%w?%w?%w?)", function(x, y)
    ...
    if x == 'n' then return "\n" .. y end
    if x == 'r' then return "\r" .. y end
    if x == 't' then return "\t" .. y end
    if x == 'u' and y and y:len() == 4 then
      return unichr(tonumber(y, 16))
    end
    return x .. y
  end)
  return stringEval, endPos  
...
function encodeString(s)
  -- NOTE(anders) added tostring in case it's not a number, test:
  -- encode{4,total=4}
  s = tostring(s)
  s = s:gsub('\\','\\\\')
  s = s:gsub('"','\\"')
  ...
  -- irc control codes
  s = s:gsub('[%z\001-\031]', function(c) return ('\\u%04x'):format(c:byte()) end)
  return s 
end
...
local _pcall = function(f, ...)
  local status, ret = pcall(f, ...)
  if not status then
    local tmp = ret:find(':', 24)
    return false, ret:sub(tmp + 1)
  else
    return ret
  end
end

...
-- Slightly modified to use Lua's bit library.
 

local function myhextodec(n)
  return tostring(tonumber('0x' .. n))
end


local Hex2Dec, BMOr, BMAnd, Dec2Hex = myhextodec, bit.bor, bit.band
...
function UUID()
        ...
        for i = 1,36 do
                if(uuid[i]==nil)then
                        -- r = 0 | Math.random()*16;
                        r = math.random (16)
                        if(i == 20 and true)then 
                                -- (r & 0x3) | 0x8
                                index = tonumber(Hex2Dec(BMOr(BMAnd((r), (3)), (8))))

...
if type(arg[1]) == 'string' then
  if arg[1]:find('[\128-\255]') then
    return idn_encode(arg[1])
  else
    return idn_decode(arg[1])
  end
else
  return {
    encode = idn_encode, decode = idn_decode,
    punycode = { encode = punycode_encode, decode = punycode_decode }
  }
end